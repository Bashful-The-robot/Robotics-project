...
class Node:
    def __init__(self, x,y,cell_cost=0,parent=None):
        self.x = x
        self.y = y
    ...
    def f_cost(self):
        return self.g_cost + self.h_cost + self.cell_cost
...
class Astar:
    ...
    def __init__(self,grid,grid_data):
    ...
    def get_vacant_neighbors(self,node):
        ...
        for i in range(-1,2):                                   #Loops through x-coordinates of grid
            for k in range(-1,2):
                ...
                if cond_x and cond_y and self.grid[x,y] <= 50:  #Inside the gridmap and not occupied
                    neighbors[Node(x,y,cell_cost=self.grid[x,y])] = self.grid[x,y]
    ...
    def get_trajectory(self,start,goal):
        ...
        start = self.flip(self.convert_to_grid(start))  #start position in grid coordinates
        ...
        #try:
        if self.grid[goal.x,goal.y]== 100:                             #Goal is on an obstacle
    ...
    #     points = self.flip(points)      #necessary for given goal positions
