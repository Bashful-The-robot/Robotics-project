...
class PathControl:
    ...
    def grid_callback(self,msg):
        ...
        if len(self.unreachableExplorerPoints) >0:
            for node in self.unreachableExplorerPoints:
                ...
                #USsed to be y,x here

        if self.geofence_points != None:
            #print("--------")
            #print(self.grid)
            #print(self.grid[47,94] == int(-1))
            #print("actual selfgrid(x,y): "+ str(self.grid[47,94]))
            #print("selfgrid(x,y): "+ str(self.grid[94,47]))
            #print("--------")
            ...
            # plt.show()
            #y= 46, x = 92
            #print("INSIDE CALLBACK, ASTAROBJ CREATED")
            data = [self.resolution,self.ymin,self.xmin]
    ...
    def generate_pathMsg(self,path):
        ...
        for node in path:
            ...
            #pose.pose.position.x = (node.x * self.resolution) + self.xmi
            #pose.pose.position.y = (node.y *self.resolution) + self.ymin
            #pose.pose.position.x  = math.ceil( ((node.x-self.xmin)/(self.xmax-self.xmin))*self.width)
            #pose.pose.position.x  = math.ceil( ((node.y-self.xmin)/(self.ymax-self.ymin))*self.height)
            pose.pose.position.x = (node.x * self.resolution) + self.ymin
            pose.pose.position.y = (node.y *self.resolution) + self.xmin
        ...
        if self.astarObj != None:
            ...
            if self.current_pos != None and self.targetPos != None:    #Checks if it's possible to create a path
                ...
                if path != None:    #it was possible to find a path
                    ...
                    print("-----------------------")
                    print("self.grid[targetPos] " + str(self.grid[self.targetPos.x,self.targetPos.y]))
