...
path_2 = "/home/robot/BASHFUL_WS/src/Voices/Voices/Passed_achieved.mp3"
global counter
counter = 0
...
class Exploration(pt.behaviour.Behaviour):
    def __init__(self):
        ...
        rospy.Subscriber("/system_flags", flags, self.flag_callback)
        self.msg_explore = False
        rospy.Subscriber('/state/cov',PoseWithCovarianceStamped, self.state_cov_callback)
        self.target = Pose()   
        self.rate = rospy.Rate(10)
        self.robot_pose = None
...
class ALLDONE(pt.behaviour.Behaviour):
    '''This is for the time limit!'''
    def __init__(self):
        super(ALLDONE, self).__init__()
    
    def update(self):
        if common_dict["DONE"]:
            rospy.loginfo("success alldone")
            playsound(path_2)
            return pt.common.Status.SUCCESS
        else:
            #print("failure timecontrol")
            rospy.logerr("failure alldone")
            return pt.common.Status.FAILURE
...
if __name__ == "__main__":
    ...
    root.add_children([TimeControl(), ALLDONE(), mainfunction])
