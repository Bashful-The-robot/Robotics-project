...
def compute_loss(
    prediction_batch: Tensor, target_batch: Tensor
    ...
    xloss = nn.functional.cross_entropy(
        prediction_batch[:,5:,:,:], 
        target_batch[:,5:,:,:],
    )

    return reg_mse, pos_mse, neg_mse, xloss
...
def train(device: str = "cpu") -> None:
    ...
    if COMPILED:
        detector = utils.compile_model(detector)

    wandb.watch(detector)

    augmented_input_transform = partial(Detector.input_transform,
                                        augment=True)

    dataset = CocoDetection(
        root="./dd2419_coco/training",
        annFile="./dd2419_coco/annotations/training.json",
        transforms=augmented_input_transform,
    )
    ...
    wandb.config.weight_c = WEIGHT_CLS
    ...
    for file_name in sorted(os.listdir(directory)):
        if file_name.endswith(".jpg"):
            ...
            #print("innan test kommit in")
            torch_image, _ = Detector.input_transform(test_image, [])
            #print("efter test kommit in")
            test_images.append(torch_image)

    if test_images:
        #print(f'TEST IMG {test_images}')
        test_images = torch.stack(test_images)
        test_images = test_images.to("cpu")#INSTEAD PF DEVICE 
    ...
    while current_iteration <= NUM_ITERATIONS:
        ...
        for img_batch, target_batch in dataloader:
            ...
            reg_mse, pos_mse, neg_mse, LT = compute_loss(out, target_batch)
            loss = WEIGHT_POS * pos_mse + WEIGHT_REG * reg_mse + WEIGHT_NEG * neg_mse + WEIGHT_CLS*LT
            ...
            if current_iteration % 250 == 0 and show_test_images: #CHANGE TO 250 LATER
                detector.eval()
                with torch.no_grad():
                    ...
                    for i, test_image in enumerate(test_images):
                        ...
                        plt.imshow(
                            ...
                            extent=(0, 640, 480, 0), #extent=(0, 1280, 720, 0),
                            alpha=0.7,
                        )

                        # add bounding boxes
                        utils.add_bounding_boxes(ax, bbs[i]) #TAKE AWAY INT(BBS..)
    ...
    utils.save_model(detector, model_path, compiled=COMPILED)
...
def validate(
    ...
    with torch.no_grad():
        count = total_pos_mse = total_reg_mse = total_neg_mse = tot_xloss = loss = 0
        image_id = ann_id = 0
        #category_id = 0
        for val_img_batch, val_target_batch in val_dataloader:
            ...
            reg_mse, pos_mse, neg_mse, xloss = compute_loss(val_out, val_target_batch)
            total_reg_mse += reg_mse
            total_pos_mse += pos_mse
            total_neg_mse += neg_mse
            tot_xloss += xloss
            loss += WEIGHT_POS * pos_mse + WEIGHT_REG * reg_mse + WEIGHT_NEG * neg_mse + WEIGHT_CLS*xloss
            imgs_bbs = Detector.decode_output(val_out, topk=100)
            for img_bbs in imgs_bbs:
                for img_bb in img_bbs:
                    #print(f"img_bb {imgs_bbs}") 
                    coco_pred.dataset["annotations"].append(
                        {
                            ...
                            "category_id": img_bb["category_id"],  # TODO replace with predicted category id
                            "score": img_bb["score"],
                            "image_id": image_id,
                        }
                    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        s = torch.cuda.memory_summary(device=None, abbreviated=False)
        #print(s)
        raise
